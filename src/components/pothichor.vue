    ...
    async loadRestaurants() {
      try {
        const req = await fetch(
          "https://www.swiggy.com/dapi/restaurants/list/v5?lat=10.51600&lng=76.21570&collection=94967&tags=layout_BAU_Contextual%2Cappam%2Clayout_ux4&sortBy=&filters=&type=rcv2&offset=0&page_type=null"
        );
        const res = await req.json();
        const data = res.data.cards;

        // ✅ Extract title & description for h1 & h5
        const headerCard = data[0]?.card?.card;
        if (headerCard) {
          this.title = headerCard.title || "Biryani";
          this.description =
            headerCard.description ||
            "Flavours wrapped in nostalgia and banana leaves";
        }
        ...
        filtered.forEach((item) => {
          ...
          offer.textContent = info.aggregatedDiscountInfoV3
